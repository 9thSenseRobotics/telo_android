...
public class UpdateWidgetService extends Service {
	...
	public void onStart(Intent intent, int startId) {
		...
		ComponentName thisWidget = new ComponentName(getApplicationContext(), RobotCommWidget.class);
		...
		for (int widgetId : allWidgetIds2) {
			if (_XMPPstatus != null) remoteViews.setTextViewText(R.id.XMPP,_XMPPstatus);
			if (_bluetoothStatus != null) remoteViews.setTextViewText(R.id.bluetooth,_bluetoothStatus);
			if (_sentToServer != null) remoteViews.setTextViewText(R.id.sentToServer,_sentToServer);
			if (_sentToRobot != null) remoteViews.setTextViewText(R.id.sentToRobot,_sentToRobot);
			appWidgetManager.updateAppWidget(widgetId, remoteViews);

		/*
		// Register an onClickListener.  to use this, set up a broadcast listener in RobotCommService
		Intent clickIntent = new Intent(this.getApplicationContext(),
				RobotCommService.class);

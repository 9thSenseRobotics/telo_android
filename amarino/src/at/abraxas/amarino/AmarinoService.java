...
public class AmarinoService extends Service {
	...
	public void onCreate() {
		...
		// add the listener for the C2DM command to this service's broadcast receiver
		filter.addAction("com.denbar.action.ROBOT_COMMMAND");
	...
	BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (AmarinoIntent.ACTION_SEND.equals(action)){
				...
				startService(intent);
			} else if ("com.denbar.action.ROBOT_COMMMAND".equals(action))
			{
				// we just got a C2DM robot command intent
				// translate that intent into one that we can use to send data
				// over bluetooth
				String cmdString = intent.getStringExtra("robotCommand");
				char cmdChar = cmdString.charAt(0);
				...
				//if (cmdChar == 'f')
				//{
				//	intent2.putExtra(AmarinoIntent.EXTRA_DATA, 254);
				//} else
				//{
				//	intent2.putExtra(AmarinoIntent.EXTRA_DATA, 0);
				//}
				//intent2.putExtra(AmarinoIntent.EXTRA_DATA, cmdChar);
